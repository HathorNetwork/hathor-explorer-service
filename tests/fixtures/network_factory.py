...
class AggregatedPeerFactory(Factory):
    ...
    entrypoints = lazy_attribute(lambda o: [f"tcp://{entrypoint()}" for i in range(fake.random_int(min=1, max=1))])
...
class AggregatedNodeFactory(Factory):
    ...
    latest_timestamp = lazy_attribute(lambda o: fake.pyint(min_value=10_000, max_value=100_000))
    entrypoints = lazy_attribute(lambda o: [f"tcp://{entrypoint()}" for i in range(fake.random_int(min=1, max=1))])
    connected_peers = lazy_attribute(lambda o: [fake.sha256() for i in range(fake.random_int(min=1, max=1))])
