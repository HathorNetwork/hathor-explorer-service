...
class TransactionFactory(factory.DictFactory):
    ...
    inputs = factory.LazyFunction(
        lambda: [TxInputFactory() for _ in range(fake.random_int(min=1, max=20))]
    )
    outputs = factory.LazyFunction(
        lambda: [TxOutputFactory() for _ in range(fake.random_int(min=1, max=20))]
    )
...
class AddressBalanceFactory(factory.DictFactory):
    ...
    success = True
    tokens_data = factory.lazy_attribute(
        lambda o: gen_tokens_data(fake.random_int(min=1, max=100))
    )
...
class AddressSearchFactory(factory.DictFactory):
    ...
    transactions = factory.LazyFunction(
        lambda: [TransactionFactory() for _ in range(fake.random_int(min=1, max=20))]
    )
...
class VersionResourceFactory(factory.DictFactory):
    version = factory.Faker(
        "ipv4"
    )  # not actuallt ipv4, but it generated some dot separated numbers
