...
class ApiGatewayEvent:
    def __init__(self, event: dict, context: LambdaContext) -> None:
        self.multiValueQueryStringParameters = event.get("multiValueQueryStringParameters") or {}
...
class ApiGateway:
    def __call__(
        self, function_to_call: Callable[[ApiGatewayEvent, LambdaContext, Any], dict]
    ) -> Callable[[dict, LambdaContext, Any], dict]:
        def wrapper(
            event: dict, context: LambdaContext, *args: Any, **kwargs: Any
        ) -> dict:
            errors_status = {
